...
class Book:
	...
	genres = ['General Fiction', 'Romance', 'Fantasy', 'Science Fiction', 
	          'Horror', 'Mystery', 'Young Adult']

	def __init__(self, gr_id, title, author, pub_date, num_ratings,
	             avg_rating, genre, id=0):
		self.id = id
	...
	def __str__(self):
		return str(self.gr_id) + ' ' + self.title[:25] + ' by ' + \
		           self.author + ' ' + self.genre + ' ' + self.avg_rating

	# TODO pull nonfiction and no_genre
	def get_books(book_ids, labeled_fiction=True, unlabeled_books=True,
	              nonfiction=False, no_genre=False):
		books = {}
		if unlabeled_books:
			f = open(Book.unlabeled, 'r', encoding='latin1')
			for line in f:
				arr = line.split('\t')
				books[arr[0]] = Book(arr[0], arr[1], arr[2], arr[3], arr[6],
                      arr[7].strip(), 'none')
			f.close()
		if labeled_fiction:
    ...
    # Takes a list of book objects		
	def save(books, file):
		f = open(file, 'a', encoding='latin1')
		for book in books:
			b = book
			f.write(str(b.gr_id) + '\t' + b.title + '\t' + b.author + '\t' +
			        b.pub_date + '\t' + '' + '\t' + '' + '\t' + 
					str(b.num_ratings) + '\t' + str(b.avg_rating) + '\n')
		f.close()
		

class User:

	ratings_file = 'C:/docs/GoodreadsDataAnalysis/data/ratings.txt'

	all_books = Book.get_books(book_ids=[])
    ...
    # Loads users that have been saved in user files
    # user files preserve previous random sampling of a user's book ratings	
	...
	def get_genre_split(books):
		...
		for book in books: 
			if book[0] != None:
				genre = book[0].genre
				if genre == 'Womens Fiction' or genre == 'Fiction':
	...
	def get_fiction_split(genre_split):
		gs = genre_split
		tf = gs['General Fiction'] + gs['Romance'] + gs['Fantasy'] + \
		     gs['Science Fiction'] + gs['Horror'] + gs['Mystery'] + \
			 gs['Young Adult']
		if tf == 0: # Avoid division by zero
			tf = 0.0000001
		fiction_split = OrderedDict([('General Fiction', gs['General Fiction'] / tf),
                               		 ('Romance', gs['Romance'] / tf),
						             ('Fantasy', gs['Fantasy'] / tf),
									 ('Science Fiction', gs['Science Fiction'] / tf),
						             ('Horror', gs['Horror'] / tf),
									 ('Mystery', gs['Mystery'] / tf),
	...
	def display_data(self):
		...
		print('sample proportion: ' + str(p) + '; labeled = ' + str(labeled) + \ 
	...
	def save_sampled(self, user_file, get_genre_file):
		...
		for sample in self.samples:
			if sample[0].genre == 'none':
				f.write(str(sample[0].gr_id) + '\t' + sample[0].title + '\t' + \
				        sample[0].author + '\t' + sample[0].pub_date + '\t' + \
						'' + '\t' + '' + '\t' + str(sample[0].num_ratings) + \
						'\t' + str(sample[0].avg_rating) + '\n')
		f.close()
